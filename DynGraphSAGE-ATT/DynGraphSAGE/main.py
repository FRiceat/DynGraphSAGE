...
parser.add_argument('--heads', type=int, default=4)  # 注意力头数
...
while args.no_stop:
    ...
    def getLoss(feed_dict, agg_neigh_list1, agg_neigh_list2):
        ...
        if args.now >= args.past:
            ...
            time_loss = torch.div(-F.logsigmoid(torch.mul(embedding1, embedding2).sum(-1)).sum(), num) / 3
            losses += time_loss
        # print('损失：', losses)
        return losses
