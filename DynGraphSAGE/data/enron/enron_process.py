...
def enron():
    ...
    with open('../data/enron/vis.graph.nodeList.json') as f:
    ...
    links = []
    ts = []
    with open('../data/enron/vis.digraph.allEdges.json') as f:
        for chunk in lines_per_n(f, 5):
            chunk = chunk.split("\n")
            # 连接的边
            name_string = chunk[2].split(":")[1]
            ...
            timestamp = dateutil.parser.parse(time_string[x[0] + 1:x[1]])
            ts.append(timestamp)
    ...
    for (a, b, time) in links:
        ...
        if slice_id not in slice_links.keys():  # 为每个时间创建快照;
            ...
            if slice_id > 0:
                slice_links[slice_id].add_nodes_from(
                    slice_links[slice_id - 1].nodes(data=True))  # 将前一时间的节点放入该时刻的图中; 节点不会消失
    ...
    for id, slice in slice_links.items():
        print("In snapshoot {:<2}, #Nodes={:<5}, #Edges={:<5}".format(id, slice.number_of_nodes(),
                                                                      slice.number_of_edges()))  # 节点和边的数量
    ...
    save_path = "../data/enron/enron.pkl"
