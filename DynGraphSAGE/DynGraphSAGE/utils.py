...
def load_data(args):
    ...
    # random_matrix = torch.rand(args.number[-1], args.first_dim)
    # node_vectors = random_matrix / torch.norm(random_matrix, dim=1, keepdim=True)
    # for i in range(len(graphs)):
    #     graphs[i].graph['feature'] = node_vectors[:args.number[i]]
    return graphs, adjs
