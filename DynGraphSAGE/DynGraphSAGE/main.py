...
parser.add_argument('--data_name', type=str, default='enron')  # 数据集名称
...
parser.add_argument('--batch_size', type=int, default=256)  # 每次训练最多节点数
...
parser.add_argument('--pos_num', type=int, default=10)  # 损失的正采样数量
...
while args.no_stop:
    ...
    def getLoss(feed_dict, agg_neigh_list1, agg_neigh_list2):
        ...
        node_pos = node_pos[0]
        node_neg = node_neg[0]
        embes_hidden1, embes_hidden2, embes = model.forward(graphs, adj_list, agg_neigh_list1, agg_neigh_list2)
        if embes == 'null':
            embes = embes_hidden2
    ...
    model.eval()
    with torch.no_grad():
        embes_hidden1, embes_hidden2, embedding = model(graphs, adj_list, agg_neigh_list1, agg_neigh_list2)
